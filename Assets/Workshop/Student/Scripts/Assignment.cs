...
namespace Assignment
{
    public class Assignment : MonoBehaviour
    {
        ...
        public void AS01_RandomItemDrop()
        {
            ...
            GameObject go = Instantiate(as01_items[randomItem]);
            Debug.Log($"Got item: {go.name}");
        }
        ...
        public void AS02_NestedLoopForCreate2DMap()
        {
            for (int y = 0; y < as02_rows; y++)
            {
                ...
                {
                    int randomTile = UnityEngine.Random.Range(0, as02_floorTiles.Length);
                    GameObject tile = Instantiate(as02_floorTiles[randomTile], new Vector2(x, y), Quaternion.identity);
                    Debug.Log(tile.name);
                }
            }
        }
        ...
        {
            for (int i = 0; i < as04_enemyHP.Length; i++)
            {
                if (i == 0)
                {
                    as04_enemyHP[i] -= as04_damage;
                    Debug.Log($"FirstEnemy hp :{as04_enemyHP[i]}");
                }
                else if (i == as04_enemyHP.Length - 1)
                {
                    as04_enemyHP[i] -= as04_damage;
                    Debug.Log($"LastEnemy hp :{as04_enemyHP[i]}");
                }
                else if (i == as04_target)
                {
                    as04_enemyHP[i] -= as04_damage;
                    Debug.Log($"TargetEnemy {as04_target} hp :{as04_enemyHP[i]}");
                }
            }
        }
        ...
        {
            for (int i = 0; i < as07_heroHPs.Length; i++)
            {
                if (i == 0)
                {
                    as07_heroHPs[i] += as07_heal;
                    Debug.Log($"FirstHero hp :{as07_heroHPs[i]}");
                }
                else if (i == as07_heroHPs.Length - 1)
                {
                    as07_heroHPs[i] += as07_heal;
                    Debug.Log($"LastHero hp :{as07_heroHPs[i]}");
                }
                else if (i == as07_targetIndex)
                {
                    as07_heroHPs[i] += as07_heal;
                    Debug.Log($"TargetHero {as07_targetIndex} hp :{as07_heroHPs[i]}");
                }
            }
        }
        ...
        {
            for (int i = 0; i < as08_dialogues.Length; i++)
            {
        ...
        public void AS11_SpawnEnemies()
        {
            for (int i = 0; i < as11_enemyHPs.Length; i++)
            {
                ...
                Debug.Log($"Spawned Enemy at position: {i} with HP: {as11_enemyHPs[i]}");
                // สมมติว่ามีสคริปต์ Enemy ที่มีตัวแปร hp
            }
        }
